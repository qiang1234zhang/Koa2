...
const bodyParser = require('koa-bodyparser');//body中间件 对于POST请求的处理，koa-bodyparser中间件可以把 koa2 上下文的formData数据解析到ctx.request.body中。
var path=require("path");

const app = new Koa();
const router = new Route();
//解析post
app.use(bodyParser());//加载
...
router.get('/', async (ctx, next) => {
    ...
    // await ctx.render("index",{data:str});//需渲染的模板名称及数据
     //4.点击a标签显示内容
     var sel= ctx.query.name || "未点击";//name是jade中href定义的请求参数
     await ctx.render("index",{data:sel});
});
